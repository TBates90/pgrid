...
from ..terrain.heightmap import smooth_field
from ..terrain.noise import fbm, domain_warp
...
def _soft_snap_macro_edges(
    g1: PolyGrid,
    e1: int,
    g2: PolyGrid,
    e2: int,
    *,
    tol_factor: float = 0.15,
) -> None:
    """Snap individual boundary vertices that are close enough.

    Unlike the full-edge overlap check used for hex-centred tiles,
    this function operates **per-vertex**: each pair of corresponding
    boundary vertices is snapped to their midpoint only if the
    distance between them is within *tol_factor* × edge-length.
    Vertices that exceed the threshold (typically near pentagon
    corners where the angular deficit concentrates) are left
    untouched.

    This closes the thin mid-edge gaps between neighbouring apron
    grids around a pentagon without injecting wedge/pinch distortion
    at the divergent corners.

    Parameters
    ----------
    g1, g2 : PolyGrid
        The two grids to snap.
    e1, e2 : int
        Macro-edge IDs of the shared boundary.
    tol_factor : float
        Maximum allowed vertex-to-vertex distance as a fraction of
        the macro-edge length for snapping.  Default 0.15.
    """
    from ..core.models import Vertex
    ...
    # Compute macro-edge length for threshold
    ...
    # Snap per-vertex where close enough
...
def build_tile_centre_only(
    coll: "DetailGridCollection",
    face_id: str,
    globe_grid: PolyGrid,
) -> "CompositeGrid":
    """Wrap the centre tile's detail grid in a CompositeGrid, no neighbours.

    Same return type as :func:`build_tile_with_neighbours` so callers
    can swap freely, but no neighbour positioning / stitching is done.
    """
    from ..building.composite import CompositeGrid, stitch_grids
    ...
    return stitch_grids({face_id: dg_center}, [])
...
def build_tile_with_neighbours(
    ...
    globe_grid: PolyGrid,
    *,
    skip_neighbour_closure: bool = False,
) -> "CompositeGrid":
    ...
    skip_neighbour_closure : bool
        If *True*, neighbours are still positioned and stitched to
        the centre grid, but neighbour↔neighbour edge closure
        (snapping + stitching) is skipped entirely.
    ...
    """
    from ..building.assembly import _position_hex_for_stitch
    from ..building.composite import CompositeGrid, StitchSpec, stitch_grids
    ...
    # When skip_neighbour_closure is set, we keep centre↔neighbour
    # stitching but omit all neighbour↔neighbour edge closure.
    if not skip_neighbour_closure:
        ...
        # use a softer closure strategy.  A 5-hex ring around a pentagon
        # has an unavoidable ~6° angular deficit per corner (120° hex
        # corner vs 108° pent corner).  Full closure via vertex averaging
        # would inject localised wedge/pinch distortion near each
        # pentagon corner.  Instead, we detect neighbour↔neighbour pairs
        # and snap only *per-vertex* where the gap is small enough,
        # leaving corner vertices (where the deficit concentrates)
        # untouched.  This closes the thin mid-edge gaps while avoiding
        # corner distortion.
        ...
        for i, n1 in enumerate(neighbours):
            ...
            for n2 in neighbours[i + 1:]:
                ...
                if is_pentagon_centre:
                    # Pentagon soft closure: snap only vertices that are
                    # close enough (per-vertex threshold).
                    _soft_snap_macro_edges(g1, e1, g2, e2)
                    outer_pairs.append((n1, e1, n2, e2))
                else:
                    # Hex-centred: only stitch if full edge overlaps well.
                    if _macro_edge_overlap_ok(g1, e1, g2, e2):
                        outer_pairs.append((n1, e1, n2, e2))

        # ── Snap outer boundary vertices to averaged positions ────
        # (For pentagon-centred tiles, _soft_snap_macro_edges already
        # handled per-vertex snapping above; skip double-snapping.)
